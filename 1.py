# 1. Develop a program to create histograms for all numerical features 
# and analyze the distribution of each feature. 
# Generate box plots for all numerical features and identify any outliers. 
# Use California Housing dataset."

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns

# Load the dataset (Note: Ensure the file 'California.xlsx' is in your directory)
df = pd.read_excel('California.xlsx')

# Display the first few rows of the dataset
print("First few rows of the dataset:")
print(df.head())

# print(df.nunique())
# print(df.describe()) 
print(df.shape)

# Create histogram for all numerical features
def plot_histograms(dataframe):
    dataframe.hist(bins=20, figsize=(15, 10), color='skyblue', edgecolor='black')
    plt.suptitle("Histograms for Numerical Features", fontsize=16)
    plt.show()

# Create box plots for all numerical values
def plot_boxplots(dataframe):
    num_columns = dataframe.select_dtypes(include=[np.number]).columns
    for column in num_columns:
        plt.figure(figsize=(10, 5))
        sns.boxplot(data=dataframe, x=column, color='lightgreen')
        plt.title(f"Box Plot of {column}", fontsize=14)
        plt.show()

# Analyze outliers for each numerical feature
def analyze_outliers(dataframe):
    num_columns = dataframe.select_dtypes(include=[np.number]).columns
    
    for column in num_columns:
        Q1 = dataframe[column].quantile(0.25)
        Q3 = dataframe[column].quantile(0.75)
        IQR = Q3 - Q1
        
        lower_bound = Q1 - 1.5 * IQR
        upper_bound = Q3 + 1.5 * IQR
        
        # logical OR operator (|) is used to find values outside the bounds
        outliers = dataframe[(dataframe[column] < lower_bound) | (dataframe[column] > upper_bound)]
        
        print(f"Features: {column}")
        print(f"Number of outliers: {len(outliers)}")
        print(f"Outliers Values: \n {outliers[column].values}\n")

# Execute functions
print("\n --- Plotting Histograms ---")
plot_histograms(df)

print("\n --- Plotting Boxplots ---")
plot_boxplots(df)

print("\n --- Analyzing Outliers ---")
analyze_outliers(df)
